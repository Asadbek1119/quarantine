...
class Menu extends \kartik\tree\models\Tree
{
    ...
    public function getUrl($params = [])
    {
        $model = $this;
        $menuParams = [];
        if ($model->url_type === 'category') $menuParams = ['category/index', 'id' => $model->url_value];
        if ($model->url_type === 'page') $menuParams = ['info/index', 'id' => $model->url_value];
        if ($model->url_type === 'file') $menuParams = ['subcategory/index', 'id' => $model->url_value];
        if ($model->url_type === 'leader') $menuParams = ['leader-category/index', 'id' => $model->url_value];
        if ($model->url_type === 'news') $menuParams = ['post-category/index', 'id' => $model->url_value];
