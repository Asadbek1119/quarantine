            ...
            <?= $form->field($node, 'url_type')->dropDownList([
                ...
                'c-action' => "Sayt bo'limlari",
                   ...
                   var t =  $(this).val()
                    ...
                    else if (t === "category" || t == "c-action" || t=="page" || t=="file" || t=="leader" || t=="news"){
                        $.ajax({
                            url: menu_url,
                            data: {type:t},
                            type: "get",
            ...
            <div class="form-group highlight-addon field-tree-url_value">
                ...
                <div id="menu-value">
                    ...
                    if ($node->url_type == 'category')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(Category::find()->all(), 'slug', 'name_uz'))->label(false);
                    elseif ($node->url_type == 'page')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(Info::find()->all(), 'slug', 'title'))->label(false);
                    elseif ($node->url_type == 'file')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(Subcategory::find()->all(), 'slug', 'name'))->label(false);
                    elseif ($node->url_type == 'leader')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(LeaderCategory::find()->all(), 'slug', 'name'))->label(false);
                    elseif ($node->url_type == 'news')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(PostCategory::find()->all(), 'slug', 'name'))->label(false);
